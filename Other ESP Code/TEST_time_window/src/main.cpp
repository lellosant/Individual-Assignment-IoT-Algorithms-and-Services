...
void TaskSampling(void *pvParameters){
    if(maxFrequency <1){
      Serial.println("Error max frequency not found, try again later");
      maxFrequency = 5; 
      //TODO
    }


    uint32_t sampleFreq = ceil(maxFrequency*2.5); //Shannon Nyquist Theorem
    ...
    if(sampleFreq< 1000){
      ...
      while (1){
        ...
        Serial.printf("the time for sampling is %d ms\n", (micros()-startWindowTime)/1000);
